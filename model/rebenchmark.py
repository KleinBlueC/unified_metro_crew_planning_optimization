...
class TrainTaskSet():
    ...
    def __init__(self, data:list) -> None:
        ...
        # for special tasks
        self.special_tasks_cnt = 0
        self.covered_special_task_cnt = 0
        self.covered_special_task_set = set() # ((depot, *task), ...) with task as (s_start, s_end, pi, normal_flag)
        self.transfer_cnt = 0
        ...
        for d in self.days:
            ...
            for o, day_depot_tasks in day_tasks.items():
                ...
                for task in day_depot_tasks:
                    if task[3]!= 1:
                        self.special_tasks_cnt += 1
    ...
    def record_special_task(self, special_task, depot):
        self.covered_special_task_cnt += 1
        special_task = (depot, *special_task)
        assert special_task not in self.covered_special_task_set
        self.covered_special_task_set.add(special_task)
...
class ReBenchmarkAlgorithmSolver():
    ...
    def run_for_reschedule(self, duty_gropups:list, train_services:list, virtual_duty_windows:dict, sorted_drivers:list, display:bool=True):
        ...
        special_task_coverage = (self.task_set_manager.covered_special_task_cnt, 
                                 self.task_set_manager.special_tasks_cnt, 
                                 round(self.task_set_manager.covered_special_task_cnt / self.task_set_manager.special_tasks_cnt, 3))
        
        return self.bm_obj, self.bm_cvg, self.bm_t, self.task_set_manager.task_cvg_dict, \
            special_task_coverage, self.task_set_manager.transfer_cnt
    ...
    def greedy_leg_for_one_driver(self, driver:Driver, reschedule_plan:list=None) -> IndividualPlan:
        ...
        def _check_meal(begin, end, selected_duty_window) -> int:
            ...
            # if min(end, selected_duty_window[0] + self.ge) - max(begin, selected_duty_window[0] + self.gb) < self.g: # not suitable for a meal
                return None
            ...
            while cur_time_unit <= selected_duty_window[1]:
                ...
                for l in lic:
                    ...
                    for o in [(l-1)*2, (l-1)*2+1]:
                        if f"{cur_depot}-{o}" not in self.transfer_dict: # added 20241214 for beijing line1-6 4-5
                            continue
                        transfer_time = self.transfer_dict[f"{cur_depot}-{o}"]
                        for task in filtered_tasks[d][o]:
                            if task[0] > cur_time_unit + transfer_time:
                                if task[3] != 1 or ( train_task_other_line is None or train_task_other_line[0] > task[0] ): # find the earliest other-line task
                ...
                if (train_task_this_line is not None and train_task_this_line[3] != 1):
                    if train_task_this_line[1] + self.beta > selected_duty_window[1] - self.eps2:  # ready to sign-out
                        ...
                        driver_plan.add_event(d, "w", train_task_this_line[0], train_task_this_line[1] + self.beta, 
                                            (self.type_cost["w"] - train_task_this_line[2])*(train_task_this_line[1] - train_task_this_line[0]) + self.type_cost["r"]*self.beta, 
                                            cur_depot, alternate_depot_fn(cur_depot)) # work and rest
                        self.task_set_manager.remove_task(d, cur_depot, train_task_this_line)
                        if train_task_this_line[3] != 1:
                            self.task_set_manager.record_special_task(train_task_this_line, cur_depot)
                        cur_time_unit = train_task_this_line[1] + self.beta
                        cur_depot = alternate_depot_fn(cur_depot)
                elif (train_task_other_line is not None and train_task_other_line[3] != 1):
                    if train_task_other_line[1] + self.beta > selected_duty_window[1] - self.eps2: # ready to sign-out
                        ...
                        self.task_set_manager.add_transfer()
                        driver_plan.add_event(d, "w", train_task_other_line[0], train_task_other_line[1] + self.beta, 
                                            (self.type_cost["w"] - train_task_other_line[2])*(train_task_other_line[1] - train_task_other_line[0]) + self.type_cost["r"]*self.beta, 
                        ...
                        if train_task_other_line[3] != 1:
                            self.task_set_manager.record_special_task(train_task_other_line, next_depot)
                        cur_time_unit = train_task_other_line[1] + self.beta
                        cur_depot = alternate_depot_fn(next_depot) 
                        cur_line = next_line
                    
                elif train_task_this_line is not None and (train_task_other_line is None or train_task_this_line[0] < train_task_other_line[0]):
                    if train_task_this_line[1] + self.beta > selected_duty_window[1] - self.eps2:  # ready to sign-out
                        ready_to_sign_out = True
                    else:
                        driver_plan.add_event(d, "w", train_task_this_line[0], train_task_this_line[1] + self.beta, 
            ...
            for line in self.lines:
                ...
                for d_idx, depot in enumerate(self.depots[line]):
                    ...
                    while s_start < last_start and s_end < self.Tw - self.eps2 - self.beta:
                        self.train_services[day_id][depot].append((s_start, s_end, pi, normal_flag))     ### here changed for the day-aware version
